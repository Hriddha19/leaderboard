...
export default function handler(req, res) {
  try {
    // Extract from query parameters instead
    const { type, data } = req.query;
    
    if (!type || !data) {
      return res.status(400).json({ 
        error: 'Missing parameters. Use: /api/image?type=goals&data=Name1+10+Name2+8' 
      });
    }

    // Parse the data string
    const dataString = data.replace('.png', '').replace('.svg', '');
    ...
    const svg = `<?xml version="1.0" encoding="UTF-8"?>
      <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
        ...
        ${leaderboard.map((entry, index) => {
          ...
          if (position === 2) badgeColor = '#C0C0C0';
          if (position === 3) badgeColor = '#CD7F32';
          if (position > 3) badgeColor = '#4A5568';
          
          return `
            <circle cx="50" cy="${y}" r="20" fill="${badgeColor}"/>
            <text x="50" y="${y + 5}" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="black" text-anchor="middle">${position}</text>
            <text x="90" y="${y + 8}" font-family="Arial, sans-serif" font-size="24" font-weight="bold" fill="white">${entry.name}</text>
            <text x="${width - 40}" y="${y + 8}" font-family="Arial, sans-serif" font-size="28" font-weight="bold" fill="#FFE135" text-anchor="end">${entry.score}</text>
            ${index < leaderboard.length - 1 ? `<line x1="40" y1="${y + 30}" x2="${width - 40}" y2="${y + 30}" stroke="rgba(255,255,255,0.2)" stroke-width="1"/>` : ''}
          `;
        }).join('')}
        
        <text x="${width/2}" y="${height - 20}" font-family="Arial, sans-serif" font-size="14" fill="rgba(255,255,255,0.5)" text-anchor="middle">Generated by Discord Bot</text>
      </svg>
    `;
    ...
    // Return SVG (Discord supports SVG images)
    res.setHeader('Content-Type', 'image/svg+xml');
    res.setHeader('Cache-Control', 'public, max-age=3600');
    res.send(svg);
